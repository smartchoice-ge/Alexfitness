...
if (!isset($mssqlconn) || $mssqlconn === false) {
    error_log("MSSQL Connection Error in send_promo_sms_geo.php (for settings)");
...
$facebookLink = "https://www.facebook.com/Alexfitnesskobulrti"; // Keep this part separate or also make it a setting
$nosms = "No2981To90775";
...
if ($httpCode >= 200 && $httpCode < 300) {
    ...
    if ($responseData !== null && isset($responseData['data'][0]['statusId']) && $responseData['data'][0]['statusId'] == 1) {
        ...
        if (isset($conn) && !$conn->connect_error) { // Check connection again before logging
            $stmt_log = $conn->prepare("INSERT INTO promo_sms_logs (client_id_num, record_date) VALUES (?, NOW())");
            if ($stmt_log) {
