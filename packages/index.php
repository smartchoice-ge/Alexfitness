...
<head>
    ...
    <title>ვადაგასული პაკეტები</title>
...
<body class="bg-gray-100">
    ...
    ?>
        ...
        <h1 class="text-2xl font-bold mb-4 text-gray-700">ვადაგასული პაკეტები</h1>

        <div class="flex"> <div id="lastPackageFiltersContainer" class="mb-4 p-4 bg-gray-50 rounded shadow w-1/2">
                    <label for="packageFilterSelect" class="font-semibold text-gray-700">ბოლო პაკეტის მიხედვით გაფილტვრა:</label>
                    <div id="lastPackageFilterDropdownContainer" class="mt-1">
                    </div>
        </div>
            <div class="w-1/2 flex justify-end items-start p-4 space-x-2">
                <a href="/packages/report.php" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    რეპორტი
                </a>
            ...
            <table id="clientListTable" class="min-w-full" style="width:100%">
                <thead class="bg-gray-100 border-b border-gray-300">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 text-sm font-medium text-gray-600 uppercase tracking-wider">ვადა გაუვიდა</th>
                        <th class="text-left px-4 py-3 text-sm font-medium text-gray-600 uppercase tracking-wider">დღეები ვადის გასვლის შემდეგ</th>
                    ...
                    <tr class="filter-row bg-gray-50">
                        ...
                        <th class="p-1">
                            <div class="filter-input-group">
                                <input type="text" id="lastPackageColumnSearch" placeholder="Search Last Pkg..." title="Filter by Last Package name">
                                <button id="applyLastPackageSearch" class="filter-apply-btn bg-green-500 text-white hover:bg-green-600">OK</button>
                            </div>
                        </th>
                        <th class="p-1"></th> 
                        <th class="p-1">
                            <div class="filter-input-group">
                                <input type="number" id="daysSinceExpiryMin" placeholder="From" title="Filter by min days since expiry (e.g., 1)" min="0" class="w-16">
                                <span class="px-1">to</span>
                                <input type="number" id="daysSinceExpiryMax" placeholder="To" title="Filter by max days since expiry (e.g., 10)" min="0" class="w-16">
                                <button id="applyDaysSinceExpirySearch" class="filter-apply-btn bg-green-500 text-white hover:bg-green-600">OK</button>
                            </div>
                        </th>
    ...
    <script>
        ...
        (function(){
            ...
            var currentSelectedLastPackage = '';
            var lastPackageColumnSearchValue = '';
            var daysSinceExpirySearchValue = '';
            ...
            $('#exportExcelBtn').on('click', function() {
                ...
                if (currentSelectedLastPackage) {
                    params.append('selectedLastPackage', currentSelectedLastPackage);
                }
                if (lastPackageColumnSearchValue) {
                    params.append('lastPackageColumnSearch', lastPackageColumnSearchValue);
                }
                if (daysSinceExpirySearchValue) {
                    if (typeof daysSinceExpirySearchValue === 'object') {
                        // For range filters, ensure both min and max are passed as an array
                        params.append('daysSinceExpiryMax[min]', daysSinceExpirySearchValue.min || '');
                        params.append('daysSinceExpiryMax[max]', daysSinceExpirySearchValue.max || '');
                    } else {
                        params.append('daysSinceExpiryMax', daysSinceExpirySearchValue);
                    }
                }
            ...
            var table = $('#clientListTable').DataTable({
                ...
                "ajax": {
                    ...
                    "type": "POST",
                    "data": function(d) {
                        d.selectedLastPackage = currentSelectedLastPackage;
                        d.lastPackageColumnSearch = lastPackageColumnSearchValue;
                        d.daysSinceExpiryMax = daysSinceExpirySearchValue;
                    },
                    "dataSrc": function(json) {
                        var filterDropdownContainer = $('#lastPackageFilterDropdownContainer');
                        ...
                        if (filterDropdownContainer.find('select.package-filter-select').length === 0) {
                            ...
                            if (json.uniqueLastPackageNames && Array.isArray(json.uniqueLastPackageNames)) {
                                json.uniqueLastPackageNames.forEach(function(pkgName) {
                ...
                "columns": [
                    ...
                    // { "data": "ActivePackage" }, // Column removed
                    { "data": "LastPackage" },
                    { "data": "LastPackageExpiryDate", "render": function(data) { return data ? data : '-'; } },
                    { "data": "DaysSinceExpiry", "render": function(data) { return data !== null ? data : '-'; } },
                ...
                "order": [ [6, 'desc'] ] // Adjusted default sort index (was 7, now 6 for Registration Date)
            });

            // ----- FILTER EVENT LISTENERS -----
            $('#lastPackageFiltersContainer').on('change', 'select.package-filter-select', function() {
                currentSelectedLastPackage = $(this).val();
                table.ajax.reload(null, true); 
            });

            $('#applyLastPackageSearch').on('click', function() {
                lastPackageColumnSearchValue = $('#lastPackageColumnSearch').val();
                table.ajax.reload(null, true); 
            });
            $('#lastPackageColumnSearch').on('keypress', function(e) { if (e.which == 13) { $('#applyLastPackageSearch').click(); } });

            $('#applyDaysSinceExpirySearch').on('click', function() {
                var minVal = $('#daysSinceExpiryMin').val();
                var maxVal = $('#daysSinceExpiryMax').val();
                daysSinceExpirySearchValue = (minVal || maxVal) ? {min: minVal, max: maxVal} : '';
                table.ajax.reload(null, true); 
            });
            $('#daysSinceExpiryMin, #daysSinceExpiryMax').on('keypress', function(e) { 
                if (e.which == 13) { $('#applyDaysSinceExpirySearch').click(); } 
            });
